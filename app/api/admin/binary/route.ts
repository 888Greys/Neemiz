...
async function requireAdmin() {
  ...
  if (!user?.email || !(await requireOwnerAdmin())) return null;
...
export async function GET(request: Request) {
  ...
  const [
    ...
    db.transaction.aggregate({
      where: { type: TransactionType.DEPOSIT, status: TransactionStatus.COMPLETED, createdAt: { gte: since, lt: until } },
      _sum: { amount: true },
      _count: true,
    }),
    db.transaction.aggregate({
      where: { type: TransactionType.WITHDRAWAL, status: TransactionStatus.COMPLETED, createdAt: { gte: since, lt: until } },
      _sum: { amount: true },
      _count: true,
    }),
    ...
    db.transaction.count({
      where: { type: TransactionType.DEPOSIT, status: TransactionStatus.FAILED, createdAt: { gte: since, lt: until } },
    }),
    ...
    db.transaction.findMany({
      where: { type: TransactionType.DEPOSIT, createdAt: { gte: since, lt: until } },
