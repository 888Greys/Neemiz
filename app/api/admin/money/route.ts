...
export async function GET(req: Request) {
  ...
  const [depositTx, withdrawalTx, p2pFeeTx, betStakes, betWins, pending, realFloat, testFloat] = await Promise.all([
    db.transaction.findMany({
      where: { type: "DEPOSIT", status: "COMPLETED", currency: "KES", provider: { in: REAL_DEPOSIT_PROVIDERS }, createdAt: { gte: since }, ...notExcluded },
      select: { createdAt: true, amount: true, provider: true },
    }),
    db.transaction.findMany({
      where: { type: "WITHDRAWAL", status: "COMPLETED", currency: "KES", provider: { in: REAL_WITHDRAWAL_PROVIDERS }, createdAt: { gte: since }, ...notExcluded },
      select: { createdAt: true, amount: true, provider: true },
    }),
    db.transaction.findMany({
      where: { provider: "p2p_fee", status: "COMPLETED", createdAt: { gte: since }, ...notExcluded },
      select: { metadata: true },
    }),
    db.transaction.aggregate({ where: { type: "BET_STAKE", status: "COMPLETED", createdAt: { gte: since }, ...notExcluded }, _sum: { amount: true } }),
    db.transaction.aggregate({ where: { type: "BET_WIN", status: "COMPLETED", createdAt: { gte: since }, ...notExcluded }, _sum: { amount: true } }),
