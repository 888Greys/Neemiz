...
export async function GET(req: Request) {
  ...
  if (!trade.pfCommitment || !trade.pfSignature || !trade.pfClientSeed || !trade.pfNonce || trade.pfPayoutMultiplier == null) {
  ...
  const terms: QuoteTerms = {
    market: trade.market,
    kind: trade.kind as QuoteTerms["kind"],
    side: trade.side as QuoteTerms["side"],
  ...
  const signature = verifyQuoteSignature(terms, trade.pfSignature)
  ...
  let replay: { won: boolean; credit: number; paid: number; exitSpot: number | null } | null = null;

  if (terminal && trade.status !== "VOID") {
    try {
      const hist = await getServerTickHistory(trade.market, trade.entryEpoch, trade.durationTicks + 50);
      const forward = hist
        .filter((h) => h.epoch > trade.entryEpoch)
        .map((h) => ({ price: h.price, epoch: h.epoch }));
      const r = verifyOutcome(terms, forward, Number(trade.stake));
      const paid = trade.status === "WON" ? Number(trade.payout) : 0;
      const matched = r.ready
        && r.won === (trade.status === "WON")
        && Math.abs(r.credit - paid) < 0.01;
      replay = { won: r.won, credit: r.credit, paid, exitSpot: trade.exitSpot == null ? null : Number(trade.exitSpot) };
      outcome = matched
