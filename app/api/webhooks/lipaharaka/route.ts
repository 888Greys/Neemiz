...
export async function POST(req: Request) {
  ...
  const reference = String(body.checkout_request_id ?? body.CheckoutRequestID ?? body.withdrawal_id ?? body.withdrawalId ?? "");
  ...
  let tx = await db.transaction.findFirst({ where: reference
    ? { reference, provider: "lipaharaka", status: { in: ["PENDING", "FAILED"] } }
  ...
  const meta = tx.metadata as { msisdn?: string; payout?: number } | null;
  const expectedAmount = tx.type === "WITHDRAWAL" ? Number(meta?.payout) : Number(tx.amount);
  if (!paid || expectedAmount !== amount || meta?.msisdn !== phone) return new Response("Rejected", { status: 422 });
  await db.$transaction(async (prisma) => {
    ...
    if (claimed.count && tx.type === "DEPOSIT") await prisma.user.update({ where: { id: tx.userId }, data: { walletBalance: { increment: tx.amount } } });
  });
