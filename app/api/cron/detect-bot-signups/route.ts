...
export async function GET(req: Request) {
  ...
  const tripped = burst || clusters.length > 0 || devices.length > 0;

  const report: BotSignupReport = { windowMinutes: windowMin, totalSignups, burst, burstThreshold, clusters, devices };
