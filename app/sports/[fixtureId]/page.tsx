    ...
    <AppShell rightPanel={<SportsBetSlip />} mainBg="bg-background">
      ...
      <div className="sticky top-0 z-30 flex items-center gap-3 border-b border-white/10 bg-[#111113] px-3 py-2.5">
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="truncate text-[13px] font-black text-white">{m.league}</span>
      ...
      <div className="mx-auto w-full max-w-3xl space-y-3 px-3 py-4 pb-10">
          ...
          <div className="px-5 pt-5 pb-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-1 flex-col items-center gap-2">
                ...
                <span className="text-center text-[13px] font-black text-white leading-tight">{m.home.name}</span>
              </div>

              {/* Score / time */}
              <div className="flex flex-col items-center gap-1 px-2">
                {m.isLive || m.home.score !== null ? (
                  <div className="flex items-center gap-2 text-[36px] font-black text-white leading-none tabular-nums">
                    <span>{m.home.score ?? 0}</span>
                    <span className="text-slate-600">:</span>
                    <span>{m.away.score ?? 0}</span>
                  </div>
                ) : (
                  <div className="text-[28px] font-black text-white">vs</div>
                )}
                {/* Period badge */}
                <span className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-[11px] font-black ${
                  m.isLive ? "bg-red-500/15 text-red-400" : "bg-white/[0.07] text-slate-400"
                }`}>
              ...
              <div className="flex flex-1 flex-col items-center gap-2">
                ...
                <span className="text-center text-[13px] font-black text-white leading-tight">{m.away.name}</span>
              </div>
            </div>

            {/* Half scores */}
            {(homePeriodScores[0] !== null || homePeriodScores[1] !== null) && (
              <div className="mt-4 rounded-xl bg-white/[0.04] px-4 py-2.5">
                <div className="grid grid-cols-[1fr_auto_auto] gap-x-6 text-[12px]">
                  <div />
                  <span className="text-center font-bold text-slate-500">1H</span>
                  <span className="text-center font-bold text-slate-500">2H</span>

                  <span className="font-bold text-white truncate">{m.home.name}</span>
                  <span className="text-center font-black text-white">{homePeriodScores[0] ?? "-"}</span>
                  <span className="text-center font-black text-white">{homePeriodScores[1] ?? "-"}</span>

                  <span className="font-bold text-white truncate">{m.away.name}</span>
                  <span className="text-center font-black text-white">{awayPeriodScores[0] ?? "-"}</span>
                  <span className="text-center font-black text-white">{awayPeriodScores[1] ?? "-"}</span>
                </div>
              </div>
            )}
          </div>
        ...
        {events.length === 0 && stats.length === 0 && homeLineup.length === 0 && (
          <div className="rounded-2xl bg-[#16171d] ring-1 ring-white/[0.07] py-16 text-center">
...
function SectionTitle({ label }: { label: string }) {
  return (
    <div className="px-4 py-3 border-b border-white/[0.07]">
      <span className="text-[13px] font-black text-white uppercase tracking-wide">{label}</span>
    </div>
  );
}
...
const EVENT_META: Record<number, { icon: string; color: string; label: string }> = {
  14: { icon: "⚽", color: "text-emerald-400", label: "Goal" },
  15: { icon: "⚽", color: "text-orange-400", label: "Own Goal" },
  16: { icon: "🟥", color: "text-red-400", label: "Red Card" },
  17: { icon: "🟨🟥", color: "text-red-400", label: "2nd Yellow" },
  18: { icon: "🔄", color: "text-blue-400", label: "Substitution" },
  19: { icon: "🟨", color: "text-yellow-400", label: "Yellow Card" },
};
...
function EventRow({ event: ev, isHome }: { event: MatchEvent; isHome: boolean }) {
  ...
  return (
    <div className={`flex items-start gap-3 py-2.5 ${isHome ? "flex-row" : "flex-row-reverse"}`}>
      {/* Minute */}
      <span className="w-10 shrink-0 text-[11px] font-black text-slate-500 tabular-nums pt-0.5 text-center">{minute}</span>
      ...
      <div className={`flex-1 min-w-0 ${isHome ? "" : "text-right"}`}>
        <div className="text-[13px] font-black text-white truncate">
          ...
          {ev.result && <span className="ml-1.5 text-[11px] font-bold text-slate-500">({ev.result})</span>}
        </div>
        {isSub && ev.related_player_name && (
          <div className="text-[11px] text-slate-500 truncate">↑ {ev.player_name}</div>
        )}
        {!isSub && ev.info && (
          <div className="text-[11px] text-slate-500 truncate">{ev.info}</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div className="mb-1 flex items-center justify-between text-[12px]">
        <span className="font-black text-white tabular-nums">{h}</span>
        <span className="font-bold text-slate-400">{stat.name}</span>
        <span className="font-black text-white tabular-nums">{a}</span>
      </div>
...
function LineupColumn({ title, logo, players }: { title: string; logo?: string; players: LineupEntry[] }) {
  ...
  return (
    <div className="px-3 pt-2">
      ...
      {bench.length > 0 && (
        <>
          <div className="my-1.5 text-[10px] font-bold text-slate-500 uppercase">Bench</div>
          {bench.map((p) => (
...
function PlayerRow({ player: p, dim = false }: { player: LineupEntry; dim?: boolean }) {
  return (
    <div className={`flex items-center gap-1.5 py-1 ${dim ? "opacity-50" : ""}`}>
      <span className="w-5 shrink-0 text-right text-[10px] font-bold text-slate-500 tabular-nums">
        {p.jersey_number ?? ""}
      </span>
