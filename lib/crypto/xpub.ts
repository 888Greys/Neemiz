...
import { evmToTron, btcP2PKHFromPubKey, ltcP2PKHFromPubKey, dogeP2PKHFromPubKey, bchCashAddrFromPubKey } from "./address-codec";
...
export function dogeAddressFromXpub(xpub: string, index: number): string {
...
export function bchAddressFromXpub(xpub: string, index: number): string {
  return bchCashAddrFromPubKey(accountNode(xpub).deriveChild(index).publicKey);
}
...
export function deriveLTCAddress(index: number): string {
  return ltcAddressFromXpub(requireXpub("MASTER_XPUB_LTC"), index);
}

export function deriveDOGEAddress(index: number): string {
  return dogeAddressFromXpub(requireXpub("MASTER_XPUB_DOGE"), index);
}

export function deriveBCHAddress(index: number): string {
  return bchAddressFromXpub(requireXpub("MASTER_XPUB_BCH"), index);
}
