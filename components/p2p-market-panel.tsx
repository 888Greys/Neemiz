...
function RateCalculator({ rates, market }: { rates: RateMap; market: MarketRateMap }) {
  const [crypto, setCrypto]  = useState<Crypto>("USDT");
  const [amount, setAmount]  = useState("");
  const [mode, setMode]      = useState<"buy" | "sell">("buy");

  const p2pRate     = mode === "buy" ? rates[crypto]?.bestBuy : rates[crypto]?.bestSell;
  const marketRate  = market[crypto]?.kes ?? null;
  const p2pResult   = p2pRate   && amount ? Number(amount) * p2pRate   : null;
  const mktResult   = marketRate && amount ? Number(amount) * marketRate : null;

  const spread = p2pRate && marketRate
    ? ((p2pRate - marketRate) / marketRate) * 100
    : null;
    ...
    <div className="bg-[#111118] border border-white/[0.06] rounded-2xl p-4 space-y-3">
      ...
      <div className="flex gap-1 bg-white/[0.04] rounded-xl p-1">
        {(["buy", "sell"] as const).map((m) => (
          ...
          >
            {m === "buy" ? "I want to Buy" : "I want to Sell"}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-4 gap-1">
        {CRYPTOS.map((c) => (
          ...
          >
            <img src={CRYPTO_ICONS[c]} alt={c} width={18} height={18} className="h-[18px] w-[18px] rounded-full" />
            {c}
          </button>
        ))}
      </div>
      ...
      <div>
        ...
        <div className="bg-white/[0.05] border border-white/[0.08] rounded-xl px-4 py-3 flex items-center gap-2 focus-within:border-[#087cff]/50 transition-colors">
          <input
            ...
            className="flex-1 bg-transparent text-white text-base font-black outline-none placeholder:text-slate-700 min-w-0"
          />
          <div className="flex items-center gap-1.5 shrink-0">
            <img src={CRYPTO_ICONS[crypto]} alt={crypto} width={16} height={16} className="h-4 w-4 rounded-full" />
            <span className="text-sm text-slate-400 font-bold">{crypto}</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="bg-white/[0.03] border border-white/[0.05] rounded-xl px-4 py-3">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] text-slate-600 font-bold">Best P2P Rate</span>
            {p2pRate && (
              <span className="text-[10px] text-slate-600">
                KSh {fmtKes(p2pRate)}/{crypto}
              </span>
            )}
          </div>
        ...
        <div className="bg-white/[0.03] border border-white/[0.05] rounded-xl px-4 py-3">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] text-slate-600 font-bold">Live Market Rate</span>
            {marketRate && (
              <span className="text-[10px] text-slate-600">
                KSh {fmtKes(marketRate)}/{crypto}
              </span>
            )}
          </div>
          <div className="flex items-baseline gap-2">
            <span className={`text-lg font-black ${mktResult ? "text-slate-300" : "text-slate-700"}`}>
...
export function P2PMarketPanel() {
  ...
  return (
    <div className="flex h-full flex-col gap-4 overflow-y-auto no-scrollbar px-3 py-5">
      ...
      {/* Rate calculator */}
      <RateCalculator rates={rates} market={market} />
